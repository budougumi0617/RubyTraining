...
feature '職員による顧客管理' do # Capybaraのときはdescribeじゃなくてfeatureブロック
  include FeaturesSpecHelper
  let(:staff_member) { create(:staff_member) }
  let!(:customer) { create(:customer) } # 「編集」がクリックされる前にテーブルに挿入されている必要があるので、定義時に生成する。

  before do
    switch_namespace(:staff)
    login_as_staff_member(staff_member) # letが呼び出されるので、テスト前に毎回StaffMemberオブジェクトができている。
  end

  scenario '職員が顧客、自宅住所、勤務先を追加する' do # scenarioはCapybara提供のitやexampleのalias
    click_link '顧客管理'
